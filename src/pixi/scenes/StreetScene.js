 ...
 * @param {{ customerCount?: number, locationLabel?: string }} [initial]
 */
export function createStreetScene(app, initial = {}) {
  ...
  const onTick = (ticker) => {
    ...
    truck.y =
      app.screen.height * 0.62 - 48 + Math.sin(elapsed * 2.2) * 1.5
    crowdLayer.children.forEach((child, i) => {
      child.y =
        app.screen.height * 0.62 -
        8 +
        Math.sin(elapsed * 3 + i * 0.7) * 2
    })
  }
